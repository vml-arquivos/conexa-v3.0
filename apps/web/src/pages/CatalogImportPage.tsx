...
import { importCatalog, getCatalogItems, type CatalogItem, type ImportResult } from '../api/catalog';
...
const CSV_MODELO = `code,name,category,unit,price,supplier
...
export function CatalogImportPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {/* Preview do catálogo atual */}
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-gray-800">Catálogo atual</h2>
          <button
            onClick={carregarPreview}
            disabled={carregandoPreview}
            className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700"
          >
            <RefreshCw className={`h-4 w-4 ${carregandoPreview ? 'animate-spin' : ''}`} />
            Atualizar
          </button>
        </div>
        ...
        {preview.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="text-sm">Catálogo vazio. Importe um arquivo para começar.</p>
            <button onClick={carregarPreview} className="text-xs text-blue-500 hover:text-blue-700 mt-2">
              Carregar catálogo
            </button>
          </div>
        ) : (
          <div className="overflow-x-auto">
                ...
                {preview.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-1.5">
                      <span className={`inline-block text-xs px-2 py-0.5 rounded-full ${item.category === 'PEDAGOGICO' ? 'bg-blue-50 text-blue-700' : item.category === 'HIGIENE' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        {item.category}
                      </span>
                    </td>
            ...
            <p className="text-xs text-gray-400 mt-2 text-right">{preview.length} produto(s) no catálogo</p>
          </div>
        )}
      </div>
    </div>
